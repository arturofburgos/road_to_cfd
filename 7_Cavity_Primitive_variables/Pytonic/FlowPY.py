...
def MakeResultDirectory(wipe=False):
    ...
    cwdir=os.getcwd()
    inter_path = os.path.join(cwdir,"7_Cavity_Primitive_variables/Pytonic")
    dir_path=os.path.join(inter_path,"Result")    #If directory does not exist, make it
...
def WriteToFile(space,iteration,interval):
    if(1/2*iteration%interval==0):
        cwdir=os.getcwd()
        inter_path = os.path.join(cwdir,"7_Cavity_Primitive_variables/Pytonic")
        dir_path=os.path.join(inter_path,"Result") 
